void __fastcall UdfxDeleteVolumeDevice(int a1, int a2, _DEVICE_OBJECT *VolumeDeviceObject)
{
  void *v3; // esi
  int v4; // edx
  ...
  if ( *((_DWORD *)v3 + 9) )
    RtlAssert(
      ...
      0);
  if ( *((_DWORD *)v3 + 4) )
    UdfxDereferenceFcb(a2, a1, (int *)&VolumeDeviceObject->Type, *((_UDF_FCB **)v3 + 4));
  if ( *((_DWORD *)v3 + 5) )
    UdfxDereferenceFcb(a2, a1, (int *)&VolumeDeviceObject->Type, *((_UDF_FCB **)v3 + 5));
